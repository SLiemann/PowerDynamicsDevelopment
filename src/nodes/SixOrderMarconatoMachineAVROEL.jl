...
@doc doc"""
```Julia
SixOrderMarconatoMachineAVROEL(Ur,Sr,H, P, D, Ω, R_a,T_ds,T_qs,T_dss,T_qss,X_d,X_q,X_ds,X_qs,X_dss,X_qss,T_AA,V0,Ifdlim,L1,G1,Ta,Tb,G2,L2)
```
...
# Keyword Arguments
- `Ur`: "Rated voltage in V, should be >0"
- `Sr`: "Rated apperent power in W, should be >0"
...
"""
@DynamicNode SixOrderMarconatoMachineAVROEL(Ur,Sr,H, P, D, Ω, R_a,T_ds,T_qs,T_dss,T_qss,X_d,X_q,X_ds,X_qs,X_dss,X_qss,T_AA,V0,Ifdlim,L1,G1,Ta,Tb,G2,L2) begin
    MassMatrix(m_int =[true,true,true,true,true,true,false,true,true,true])
end begin
    @assert Ur > 0 "Rated voltage in V, should be >0"
    @assert Sr > 0 "Rated apperent power in W, should be >0"
