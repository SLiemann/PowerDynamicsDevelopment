...
@DynamicNode GFMCurrentPrio(Sbase,Srated,p0set,q0set,u0set,Kp_droop,Kq_droop,ωf_P,ωf_Q,xlf,rf,xcf,Kp_u,Ki_u,Kp_i,Ki_i,imax,Kvi,σXR,K_vq,imax_csa,iprio,p_ind) begin
    ...
    @assert iprio == "d" || iprio == "q" || iprio == "none" "Current priorisation should be either d,q or none"
    ...
    elseif iprio == "d" #HIER FEHLT NOCH DAS VORZEICHEN!!!!
        idset_csa = IfElse.ifelse(idset > imax_csa,sign(idset)*imax_csa, idset)
        iqset_csa = IfElse.ifelse(iset_abs > imax_csa,sign(iqset)*sqrt(imax_csa^2 - idset_csa^2),iqset)
    else #equal to iprio = "q"
        iqset_csa = IfElse.ifelse(abs(iqset) > imax_csa, sign(iqset)*imax_csa, iqset) 
        idset_csa = IfElse.ifelse(iset_abs > imax_csa,sign(idset)*sqrt(imax_csa^2 - iqset_csa^2),idset)
    end
