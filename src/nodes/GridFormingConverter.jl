...
"""
=#
@DynamicNode GridFormingConverter(Sbase,Srated,p0set,q0set,u0set,Kp_droop,Kq_droop,ωf,xlf,rf,xcf,Kp_u,Ki_u,Kp_i,Ki_i) begin
...
end [[θ,dθ],[ω,dω],[Q,dQ],[e_ud,de_ud],[e_uq,de_uq],[e_id,de_id],[e_iq,de_iq],[umabs,dumabs],[umangle,dumangle]] begin
    ...
    dQ = qmeas * ωf - Q * ωf
    Uset = (q0set - Q) * Kq_droop + u0set

    #Virtual Impedance
    #TO-DO

    udset = Uset #real(Uset * (cos(θ) + 1im * sin(θ)))
    uqset = 0.0 #imag(Uset * (cos(θ) + 1im * sin(θ)))
    #Voltage control
    de_ud = udset - udmeas
    de_uq = uqset - uqmeas
